...
- [x] 2.2 Deploy + reload tmux; press `prefix + B` and confirm a ~90% popup opens running `beans tui` in the active pane's directory.
- [x] 2.3 Confirm the popup closes when `beans tui` exits, and that `prefix + b` (status-bar toggle) and other binds still work.
