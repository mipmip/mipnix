...
- [ ] 1.1 In `modules/USERS/pim/programs/huphop/default.nix`, add a `let` binding
...
- [ ] 1.2 Verify the wrapper logic: create session `<short>-><owner>` + window
...
- [ ] 2.1 In the same module, build the config attrset and render it with
      `(pkgs.formats.yaml {}).generate "config.yaml" cfg`, installed via
      `xdg.configFile."huphop/config.yaml".source`.
- [ ] 2.2 Reproduce the current working config verbatim: `base_dir: ~`,
...
- [ ] 2.3 Set the multiplex `switch_command` to
      `'${hup-tmux-switch}/bin/hup-tmux-switch' '{{.Short}}' '{{.OwnerLower}}' '{{.Repo}}' '{{.Target}}'`.

## 3. tmux binding

- [ ] 3.1 In `modules/USERS/pim/programs/tmux/default.nix`, add
      `bind G popup -E -w 80% -h 80% 'hup tui --mode multiplex --flatlist'`
      next to `bind S popup -E smg`.

## 4. Verify

- [ ] 4.1 `nix build`/flake eval succeeds (module evaluates; note the huphop
      module file must be git-tracked for the flake to see it).
- [ ] 4.2 After `home-manager switch`, `~/.config/huphop/config.yaml` is a store
      symlink and `hup config check` passes.
- [ ] 4.3 Manual: `prefix + G` opens the flat repo list; selecting an uncloned
