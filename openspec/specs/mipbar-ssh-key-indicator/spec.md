...
mipbar SHALL show an always-visible indicator of the user's ed25519 ssh-agent key state
(loaded / unloaded / stale / no-agent), let the user load or unload the key via a
click-through popover with a GUI passphrase prompt scoped to that action, and keep the
displayed state current through periodic polling backed by a plain OpenSSH ssh-agent.
