...
polkit-mediated authentication requests can be presented to the user. The agent
(`hyprpolkitagent`) SHALL be launched directly from the session environment via
`exec-once` rather than its systemd user unit, and the static `autostart.conf`
SHALL reference a stable PATH wrapper name rather than a nix-store path, so the
