...
mipbar SHALL surface windows minimized to the `special:minimized` workspace through
a bar indicator that appears only when windows are minimized and shows their count,
a click-through popover listing each minimized window (app icon and title), and
actions to restore a single window or all windows to the current workspace — keeping
the indicator's visibility and count current as windows are minimized and restored.
