# Voorzetramenshop Hosting

## Purpose

Host the voorzetramenshop webshop on durer: wire it in as a private flake
input, import its NixOS module and configure the service, supply its runtime
environment via an agenix secret, compose its nginx/ACME with durer's existing
configuration, and establish the build-on-lego2 deployment model (deploy-rs
node, `RUNME.d` wrapper, trusted-users fix, and GC policy) together with the
version-bump deploy and rollback workflow.
