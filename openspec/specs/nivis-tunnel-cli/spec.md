...
uses for every subcommand, even though the package publishes the binary as
`tunnel`.
...
The system SHALL NOT place a command named `tunnel` on the system PATH. The name
is too generic to claim system-wide for one tool.
