...
Surface and control which physical monitor each workspace is bound to: show a
per-monitor accent underline on every workspace button, react to monitor and
binding changes, and let the user rebind a workspace to another connected
monitor at runtime via a right-click screen picker.
